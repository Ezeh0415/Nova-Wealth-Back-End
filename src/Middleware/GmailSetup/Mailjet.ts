...
export class MailSender {
  ...
  public async dobuleSendEmail(to: string, subject: string, html: string): Promise<any> {
    try {
      const result = await this.mailjetClient
        .post("send", { version: "v3.1" })
        .request({
  ...
  public async sendUserDeposit(to: string, userId: object | string, type: string, currency: string, requestedAmount: number, status: string, createdAt: Date, userEmail: string, userFullName: string): Promise<any> {
  ...
  public async confirmDeposit(to: string, userId: object | string, type: string, currency: string, creditedAmount: number, status: string, creditedAt: Date, userEmail: string, userFullName: string, transactionId: string, isFirstDeposit: boolean): Promise<any> {
